...
void FDMEulerExplicit::calc_inner_domain(){
    ...
    for (unsigned long j = 1; j < _J - 1; j++){
        ...
        _alpha = dt_sig - dt_sig_2;
        _beta = _dx * _dx - (2.0 * dt_sig) + (_dt * _dx * _dx * _pde->zero_coeff(_prev_t, _x_values[j]));
        _gamma = dt_sig + dt_sig_2;

        _new_result[j] = ((_alpha * _old_result[j - 1]) +
                          (_beta * _old_result[j]) +
                          (_gamma * _old_result[j + 1])) / (_dx * _dx) -
