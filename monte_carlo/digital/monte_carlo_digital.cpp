#ifndef __MONTE_CARLO_DIGITAL_CPP
#define __MONTE_CARLO_DIGITAL_CPP

#include "monte_carlo_digital.hpp"
#include <iostream>

MonteCarloDigital::MonteCarloDigital(const unsigned long num_sims,
                                       Option *pOption,
                                       StatisticalDistribution *sd)
    : MonteCarloBase(num_sims, pOption, sd) {}
MonteCarloDigital::~MonteCarloDigital() {}

double MonteCarloDigital::calc_S_drift(const double &S) const
{
    return S * exp((_pOption->_r - 0.5 * (_pOption->_v) * (_pOption->_v)) * (_pOption->_T));
}

double MonteCarloDigital::calc_S_stochastic(const double &spot_normal) const
{
    return exp(_pOption->_v * pow(_pOption->_T, 0.5) * spot_normal);
}

double MonteCarloDigital::monte_carlo_sim(const double &S_0)
{
    double S_cur = 0.0;
