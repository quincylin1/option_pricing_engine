#ifndef __MONTE_CARLO_EUROPEAN_CPP
#define __MONTE_CARLO_EUROPEAN_CPP

#include "monte_carlo_european.hpp"
#include <iostream>

MonteCarloEuropean::MonteCarloEuropean(const unsigned long num_sims,
                                       Option *pOption,
                                       StatisticalDistribution *sd)
    : MonteCarloBase(num_sims, pOption, sd) {}
MonteCarloEuropean::~MonteCarloEuropean() {}

double MonteCarloEuropean::calc_S_drift(const double &S) const
{
    return S * exp((_pOption->_r - 0.5 * (_pOption->_v) * (_pOption->_v)) * (_pOption->_T));
}

double MonteCarloEuropean::calc_S_stochastic(const double &spot_normal) const
{
    return exp(_pOption->_v * pow(_pOption->_T, 0.5) * spot_normal);
}

double MonteCarloEuropean::monte_carlo_sim(const double& S_0)
{
    double S_cur = 0.0;
